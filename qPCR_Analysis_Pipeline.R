# =========================================================================
# Script: Advanced qPCR Analysis Pipeline (Livak Method)
# Developer: Hossein Noorollahi (ORCID: 0000-0001-5685-8821)
# Date: June 2026
# Description: Fully automated workflow for qPCR data analysis.
#              Capabilities include: Data Parsing, Relative Quantification 
#              (2^-ddCt), Welch's t-test, 95% Confidence Intervals, 
...
# USER NOTE: 
# Place your dataset in the same directory as this script.
# Default is set to the provided dummy dataset ("dummy_qPCR_data.csv").
# To analyze your own data, simply change the file name below.
input_file <- "dummy_qPCR_data.csv"

# Error handling: Check if file exists before proceeding
if (!file.exists(input_file)) {
  stop(paste("ERROR: Data file '", input_file, "' not found. Please check your working directory.", sep=""))
}
...
# Define target reference gene and control group
ref_gene <- "GAPDH"
...
# Calculate Mean Ct for Reference Gene
df_ref_mean <- df_raw %>%
  filter(Target == ref_gene) %>%
  group_by(Sample) %>%
  summarise(Ref_Mean_Ct = mean(Ct), .groups = 'drop')

# Calculate dCt for Target Genes
df_analysis <- df_raw %>%
  filter(Target != ref_gene) %>%
...
for (trt in treatments) {
  for (g in target_genes) {
    ...
    # Perform Welch's t-test if sufficient replicates exist
...
message("Statistical Analysis Complete. Results saved to 'qPCR_Detailed_Analysis.csv'.")
...
message("\n--- Full Pipeline Finished. All plots successfully generated. ---")
